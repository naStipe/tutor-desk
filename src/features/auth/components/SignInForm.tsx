...
export function SignInForm() {
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white border border-slate-200 rounded-xl p-6 sm:p-8 shadow-xs">
      <div className="mb-6">
        <h1 id="signin-heading" className="text-xl font-bold text-slate-900 tracking-tight">
          Tutor Sign In
        </h1>
        <p className="text-sm text-slate-500 mt-1">
          Enter your email and password to access your tutor account.
        </p>
      </div>

      {serverError && (
        <div
          id="signin-server-error"
          role="alert"
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        <div>
          ...
          />
          {fieldErrors.email && (
            <p id="signin-email-error" className="mt-1 text-xs text-rose-600 font-medium">
              {fieldErrors.email}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {fieldErrors.password && (
            <p id="signin-password-error" className="mt-1 text-xs text-rose-600 font-medium">
              {fieldErrors.password}
            </p>
          )}
        </div>
